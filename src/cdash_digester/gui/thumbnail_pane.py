...
def _render_pdf_page(filepath: Path) -> QPixmap:
    print(f"DEBUG _render_pdf_page: {filepath}", flush=True)
    try:
        doc = fitz.open(str(filepath))
        page = doc[0]
        pix = page.get_pixmap(matrix=fitz.Matrix(1, 1))
        doc.close()
        print(f"DEBUG _render_pdf_page: pixmap {pix.width}x{pix.height}", flush=True)
        qi = QImage(pix.samples, pix.width, pix.height,
                    pix.stride, QImage.Format_RGB888)
        pm = QPixmap.fromImage(qi)
        result = pm.scaled(_THUMB, _THUMB, Qt.KeepAspectRatio,
                         Qt.SmoothTransformation)
        print(f"DEBUG _render_pdf_page: done, null={result.isNull()}", flush=True)
        return result
    except Exception as e:
        print(f"DEBUG _render_pdf_page: exception {e}", flush=True)
        return _blank()


def _render_image(filepath: Path) -> QPixmap:
    print(f"DEBUG _render_image: {filepath}", flush=True)
    try:
        ...
        if pm.isNull():
            print("DEBUG _render_image: QPixmap is null, returning blank", flush=True)
            return _blank()
        print(f"DEBUG _render_image: done {pm.width()}x{pm.height()}", flush=True)
        return pm
    except Exception as e:
        print(f"DEBUG _render_image: exception {e}", flush=True)
        return _blank()
...
class ThumbnailPane(QScrollArea):
    ...
    def load_media(self, rows, batch_root: Path):
        ...
        print(f"DEBUG load_media: {len(rows)} rows, batch_root={batch_root}", flush=True)
        ...
        print("DEBUG load_media: cleared old widgets", flush=True)

        vp_width = self.viewport().width()
        col_count = max(1, (vp_width - 16) // (_THUMB + _SEL_BORDER * 2 + 8))
        print(f"DEBUG load_media: viewport_width={vp_width}, col_count={col_count}", flush=True)

        for i, row in enumerate(rows):
            try:
                filepath = batch_root / row["filepath"]
                print(f"DEBUG load_media [{i}]: filepath={filepath} exists={filepath.exists()}", flush=True)
                pm = _make_thumbnail(filepath) if filepath.exists() else _blank()
                print(f"DEBUG load_media [{i}]: pixmap ready, null={pm.isNull()}", flush=True)
                widget = _Thumb(row["media_id"], pm, row["ready"], row["filename"])
                print(f"DEBUG load_media [{i}]: _Thumb created", flush=True)
