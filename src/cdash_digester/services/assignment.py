...
class AssignmentService:
    ...
    def assign_media_to_doc(self, media_ids: List[int], place_id: int,
        ...
        if not place_no_change:
            p_status, place_name = dig.validate_place(place_id)
            if "Valid" not in p_status:
                dig.log(f"Place validation failed: {p_status}", "error")
                return False
        ...
        first = dig.db.get_media(media_ids[0])
        if not first:
            return False
        ...
        docs = dig.db.get_docs_for_folder(item_set_id)
        ...
        try:
            if is_multi_page:
                ...
                for page_num, media_id in enumerate(sorted(media_ids), start=1):
                    ...
                    dig.db.assign_media_to_doc(media_id, doc_id, page_num,
                                               batch_media_id)
            ...
            dig.db.recalculate_folder_status(item_set_id)
            dig.db.recalculate_batch_ready()
            return True

        except Exception as exc:
            dig.log(f"assign_media_to_doc failed: {exc}", "error")
            return False
    ...
    def _rename_media(self, media_id: int, place_id: int, place_slug: str,
        ...
        if not row:
            return
        old_path = dig.batch_path / row["filepath"]
        new_name = (
        ...
        try:
            ...
            dig.db.update_media_filename(
                media_id, new_name,
                str(new_path.relative_to(dig.batch_path)),
            )
        except OSError as exc:
            dig.log(f"  Rename failed for {row['filename']}: {exc}", "error")
