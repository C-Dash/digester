...
from ..constants import PLACE_PROP_KEYS
...
class ValidationService:
    ...
    def resolve_folder_name(self, item_set_id: int,
        ...
        if cache and cache["status"] == "valid":
            ...
            self._session.log(f"  Validated folder (cached): {name}", "info")
            return name, True

        api_status, api_name = self._session.validator.validate_folder(item_set_id)
        if "Valid" in api_status:
            self._session.log(f"  Validated folder: {api_name}", "info")
            db.upsert_folder_cache(item_set_id, api_name, folder_index, "valid")
            return api_name, True

        self._session.log(f"  Folder ID {item_set_id}: {api_status}", "warning")
        return None, False
    ...
    def place_associated_with_folder(self, place_id: int,
        ...
        """
        place = self._session.db.get_place(place_id) if self._session.db else None
        raw = (place.item_set_ids or "").strip() if place else ""
        if not raw:
            return True
